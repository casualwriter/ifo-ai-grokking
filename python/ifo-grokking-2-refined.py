"""
IFO Grokking Experiment V2: Refined Topological Probe
Focus: Tracking Max H1 Persistence in both Embedding Space and Activation Space.
...
def get_max_h1_persistence(points, max_pca_components=5):
    """
    Standardize, optionally reduce dimension, and compute the maximum persistence 
    of H1 (loops). A high value indicates a robust, large topological loop.
    """
    # 1. Standardize to make scale-invariant
    scaler = StandardScaler()
    points_scaled = scaler.fit_transform(points)
    
    # 2. PCA to avoid curse of dimensionality if needed
    ...
    # 4. Extract max persistence (Death - Birth) for H1
    max_persistence = 0.0
    if len(dgms) > 1 and len(dgms[1]) > 0:
        # Filter out any infinite death points (though H1 shouldn't have them in finite sets)
...
def train_and_track(model, train_data, val_data, p, 
                    steps=8000, lr=1e-3, weight_decay=1.0,
                    track_interval=100):
    ...
    print("Starting Refined V2 Training...")
    for step in tqdm(range(steps)):
        ...
        if step % track_interval == 0:
            ...
            with torch.no_grad():
                ...
                # --- Probe 1: Embedding Space Topology (97 points in 128D) ---
                embed_weights = model.embed.weight.cpu().numpy()
                embed_h1 = get_max_h1_persistence(embed_weights, max_pca_components=3)
                
                # --- Probe 2: Subsampled Activation Space Topology (400 points) ---
                # We combine train and val, then subsample to keep TDA fast and clean
                ...
                # Early stop if fully grokked to save time
                if val_acc > 0.995 and step > 3000:
                    print("Fully Grokked! Stopping early.")
                    break
...
def plot_results(history):
    ...
    ax.set_title('Grokking Dynamics (Accuracy)')
    ax.legend()
    ...
    ax.set_title('Embedding Space: Circularity (IFO Key Probe)')
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title('Activation Space: Circularity')
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('ifo-grokking-v2-results.png', dpi=150)
    print("\nRefined plot saved to: ifo-grokking-v2-results.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    # 8000 steps is usually more than enough for p=97 with AdamW + WD=1.0
